...
from rom_editors import character_editor
...
def randomizer_handler(app):
    ...
    filters = [kind for kind in {"playable", "boss", "other", "class"} if not CONFIG["randomize"]["stats"]["bases"][kind]["enabled"]]
    class_mode = CONFIG["randomize"]["classes"]["mode"]
    mix_promotes = CONFIG["randomize"]["mix_promotes"]["enabled"]

    updated_rom = character_editor.randomize(app.labels["status"], fe8_config, rom_data, filters, class_mode, mix_promotes)
